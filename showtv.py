...
# Engelleri aşmak ve ciner API'sinin yanıt vermesini sağlamak için gerçekçi başlıklar
HEADERS = {
    ...
    try:
        ...
        for a_tag in soup.find_all("a", href=True):
            ...
            if series_slug in href and "bolum" in href:
                match = re.search(r'bolum/(\d+)', href) or re.search(r'bolum-(\d+)', href)
                if match:
...
def verify_and_get_video_url(series_name, episode_num):
    """Doğrudan Ciner Medya'nın arka plan API'sinden imzalı m3u8'i söker"""
    try:
        ...
        # Hedef url kalıplarını senin muhtemel ask.txt dosyasındaki güncel yapıya sabitledik
        target_urls = [
        ...
        for url in target_urls:
            try:
                ...
                # Senin dosyandaki kesin eşleşen data-id özniteliğini avlıyoruz
                ...
                # Video ID bulunduysa, F12'deki tokenlı linkleri dağıtan ana merkeze vuruyoruz
                if video_id:
                    ...
                    if r_api.status_code == 200:
                        ...
                        # Yedek olarak mp4 varsa m3u8'e çevirip dön
...
def update_showtv():
    print("🚀 ShowTV HIZLI GÜNCELLEYİCİ (ICRAAT MODU)")
    print("=" * 60)
    ...
    # Sadece ilk 5 güncel diziyi kontrol listesine dahil et
